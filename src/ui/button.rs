...
#[require(Node)]
/// A UI element that will send out events [`ButtonMessage`] when it is pressed.
///
...
fn on_add_button(
    ...
    mut q_added_button: Query<(Entity, &CosmosButton, &mut Node), Added<CosmosButton>>,
) {
...
fn on_change_button(
    ...
    for (ent, btn, image, button_text, &interaction, mut bg_color) in
        q_changed_button.iter_mut().filter(|x| !x.1.is_added())
    {
        fn calc_text_color(
            btn: &CosmosButton,
            interaction: Interaction,
            text_style: &mut TextColor,
        ) {
        ...
        if let Some(button_text) = button_text {
            if let Some((new_text_value, text_font, text_color)) = &btn.text {
                if let Some((_, _, cur_text_value, cur_font_style, cur_text_color)) =
                    writer.get(button_text.0, 0)
                {
                ...
                commands.entity(button_text.0).despawn();
...
pub(super) fn register(app: &mut App) {
    app.configure_sets(
        ...
        (
            ...
            ButtonUiSystemSet::SendButtonMessages,
        )
            .chain(),
    )
    .add_systems(
